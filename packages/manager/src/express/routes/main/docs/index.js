...
function addRouters (router) {
  router.get('/docs/browser_service', async function (req, res) {
    ...
    if (query.text) {
      ...
      const queryTotal = `SELECT RAW COUNT(*) total_row from \`${connClass.projectBucketName}\` WHERE type is not missing ${filter}`
      const queryRows = `SELECT META().id, META().cas \`key\` from \`${connClass.projectBucketName}\` WHERE type is not missing ${params} ORDER BY META().cas DESC LIMIT ${limit}`
      const promises = [
  ...
  router.get('/docs/browser_sdk', async function (req, res) {
    ...
    if (query.text) {
      ...
      const promises = [
        couchViews.exec(connClass.projectBucketName, 'list_docs_all', connClass.projectBucket, options),
        couchQueries.exec(queryTotal, connClass.cluster),
      ]
      const [data, resp2] = await allSettled(promises)
      if (data.results && resp2.results) {
  ...
  router.get('/docs/browser', async function (req, res) {
    ...
    const params = `${cursor}${filter}`
    const queryTotal = `SELECT RAW COUNT(*) total_row from \`${connClass.projectBucketName}\` WHERE type is not missing ${filter}`
    const queryRows = `SELECT META().id, META().cas \`key\`, TOBOOLEAN(date_closing) noChannel from \`${connClass.projectBucketName}\` WHERE type is not missing ${params} ORDER BY META().cas DESC LIMIT ${limit}`
    const promises = [
      couchQueries.exec(queryRows, connClass.cluster),
      couchQueries.exec(queryTotal, connClass.cluster),
    ]
